...
export const toolInputSchemas = {
    ...
    generation_get_status: z.object({ scope: z.enum(["all", "canvas", "image"]).optional(), taskId: z.string().optional(), nodeIds: z.array(z.string()).optional(), limit: z.number().optional() }),
    ...
    assets_list: z.object({ kind: z.enum(["all", "text", "image"]).optional(), keyword: z.string().optional(), page: z.number().optional(), pageSize: z.number().optional() }),
...
export const toolDescriptions: Record<ToolName, string> = {
    site_navigate: "跳转网站页面。path 可为 / (首页)、/canvas (我的画布)、/canvas/:id (指定画布)、/image (生图工作台)、/prompts (提示词库)、/assets (我的素材)、/config (配置)。操作画布前若不在画布页，先用本工具打开画布。",
    ...
    canvas_create_node: "创建任意类型节点：text、image、config、audio。适合创建占位图、媒体占位、配置节点或自定义 metadata 节点。",
    ...
    canvas_run_generation: "触发指定节点生成，通常用于配置节点或文本/图片/音频节点。",
    generation_get_status: "查询当前活动网页的生成任务状态。默认返回画布和生图工作台最近任务；可用 scope 过滤来源，用 taskId 查询工作台任务，用 nodeIds 查询画布节点。",
    ...
    assets_list: "列出用户「我的素材」，支持 kind（text/image）过滤、keyword 搜索和 page/pageSize 分页。为控制体积不返回图片原始 data，仅返回封面与元信息。",
