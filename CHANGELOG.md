...
+ [新增] 右上角与导航配置入口新增 API Key 客服引导弹窗与微信二维码，并适配手机端布局。
+ [新增] 提示词来源新增 Freestylefly GPT Image 2 内置来源。
+ [新增] 新增匿名会话 Node API、加密多中转站配置、SQLite 调用摘要及同域 Responses、图片生成和图片编辑网关。
+ [调整] 主应用与文档站统一为 IonAiLabs 品牌，并移除视频能力及主应用文档、GitHub 入口。
+ [调整] 移除本地 Agent、Codex App 插件、版本更新跳转和 AI 音频生成接口，保留本地音频节点的上传与播放。
+ [调整] 前端不再保存 Key 或直连模型服务，统一使用服务端 OpenAI 兼容渠道，并移除 WebDAV、原生 Gemini、原生火山方舟和自定义请求脚本。
+ [修复] 修复本地开发时客服二维码被 API 代理拦截，以及 Node API 因无效类型依赖无法安装的问题。
+ [修复] 升级服务端 Drizzle ORM，修复公开的 SQL 标识符注入风险。
