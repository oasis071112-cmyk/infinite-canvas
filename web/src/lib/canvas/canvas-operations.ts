...
export function applyCanvasOperations(snapshot: CanvasSnapshot, ops?: CanvasOperation[]) {
    ...
    (Array.isArray(ops) ? ops : []).forEach((op, index) => {
        ...
        if (op.type === "connect_nodes") {
            const exists = connections.some((connection) => connection.fromNodeId === op.fromNodeId && connection.toNodeId === op.toNodeId);
    ...
    return { nodes, connections, selectedNodeIds, viewport };
}
