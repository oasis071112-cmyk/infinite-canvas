export default {
    ...
    apiErrors: { requestFailed: "请求失败", requestCanceled: "请求已取消", baseUrlRequired: "请先保存渠道", apiKeyRequired: "请先配置渠道和模型", authenticationFailed: "鉴权失败，请检查 API Key、套餐权限或模型权限", rateLimited: "请求被限流或额度不足，请稍后重试", notFound: "接口地址不存在（404），请检查 Base URL 和模型选择", badGateway: "网关错误（502），接口服务暂时不可用，请稍后重试", serviceBusy: "服务繁忙（503），请稍后重试", httpFailed: "请求失败（HTTP {{status}}），请检查 Base URL 和 API Key 是否正确", htmlError: "服务返回了 HTML 错误页面（{{preview}}）", invalidImageSizeFormat: "图像尺寸格式不支持，请使用 auto、9:16 或 1024x1024", positiveImageRatio: "图像比例必须是正数，例如 9:16", imageRatioLimit: "图像宽高比不能超过 3:1，请调整尺寸", positiveImageDimensions: "图像尺寸必须是正整数，例如 1024x1024", imageDimensionStep: "图像尺寸的宽高必须是 16 的倍数，请调整尺寸", imageEdgeLimit: "图像尺寸最长边不能超过 3840px，请调整尺寸", imagePixelLimit: "图像总像素需在 655360 到 8294400 之间，请调整尺寸", unknownImageResponse: "接口没有返回可在本地保存的 Base64 图片（字段：{{fields}}）", noImageReturned: "接口没有返回图片，请检查提示词是否触发安全审核或模型是否支持该操作", noContent: "没有返回内容", modelReadFailed: "读取模型失败", referenceImageReadFailed: "参考图读取失败，请换一张图片或重新上传", invalidReferenceAudio: "参考音频必须是公网 URL、资产 ID，或本地已保存的音频", localAssetReadFailed: "读取本地资产失败" },
    prompts: {
        title: "提示词中心",
        library: "提示词库",
    ...
    canvas: {
        ...
        plugins: { title: "节点插件", installedPlugin: "已安装插件 {{name}}", installed: "已安装 {{name}}", installFailed: "安装失败：{{error}}", enabled: "已启用", disabled: "已禁用", upgradeAvailable: "有新版本，点击升级", updateFromSource: "从来源更新", updated: "已更新", uninstallTitle: "卸载该插件？", uninstall: "卸载", newVersion: "有新版本可升级", officialDescription: "本项目官方插件，来自仓库注册表", refresh: "刷新", loadFailed: "加载失败：{{error}}", loadingOfficial: "正在获取官方插件…", noOfficial: "暂无官方插件", install: "安装", urlPlaceholder: "输入插件 JS 文件 URL，例如 https://.../plugin.js", noThirdParty: "还没有安装第三方插件", official: "官方插件", local: "本地插件", thirdParty: "第三方插件", warning: "插件代码会在当前页面内直接执行，可访问浏览器本地画布和媒体，也可调用当前会话已有的 AI 功能，但不能读取服务端加密的渠道 Key。请仅安装可信插件。", aiConfigRequired: "AI 配置未就绪，请先在设置里配置模型与密钥", interactiveTitle: "当前：交互中。点击切回「移动」——拖动可移动节点", movableTitle: "当前：可移动。点击切到「交互」——可操作节点内容（如转动全景）", move: "移动", interact: "交互" },
    ...
    config: {
        ...
        description: "安全渠道、模型选择与浏览器本地存储",
        modalDescription: "渠道密钥由同域后端加密保存，画布和媒体仍保存在当前浏览器",
        sessionUnavailable: "无法连接本地 API 服务",
        apiKeyContact: {
            title: "获取专属 API Key",
        ...
        localStorage: {
            ...
            description: "查看 IonAiLabs无限画布在浏览器中保存的数据量，并按对象仓库统计内容体积。",
        ...
        fileSecurity: "导入导出仅包含本地偏好和提示词来源，不包含渠道或 API Key。",
        import: "导入配置",
        export: "导出配置",
        imported: "配置与用户偏好已导入",
        ...
        channels: {
            description: "每个渠道使用 OpenAI 兼容协议；API Key 加密保存在后端，不进入浏览器持久化存储。",
            add: "新增渠道",
            unnamed: "未命名渠道",
        ...
        channelEditor: {
            ...
            baseUrl: "接口地址",
            models: "渠道模型",
            modelDescription: "已选 {{count}} 个；请为每个模型指定生图或文本能力。",
            selectModels: "选择模型",
            empty: "点击「选择模型」拉取或手动增加模型。",
            required: "请填写渠道名称、HTTPS 接口地址和 API Key",
            saveFailed: "保存渠道失败",
            httpsOnly: "仅允许公网 HTTPS 地址；后端会自动补充 /v1。",
            keepKey: "留空则继续使用已加密保存的 Key",
            keySecurity: "Key 仅通过 HTTPS 提交，不会写入浏览器存储或返回前端。",
            capabilities: {
                image: "生图",
                text: "文本",
            },
