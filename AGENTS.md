...
- 前端请求统一放在 `web/src/services/api/`；AI 模型请求只访问同域 `/api`，由项目 Node 后端连接 OpenAI 兼容中转站，提示词仓库等明确保留的公共数据源可由浏览器直接读取。
...
- 左侧画布面板等列表里的节点/元素缩略图容器，非图片类型（文本、配置、音频等）不要使用 `theme.node.fill`（`#e7e5df`/`#292524`）这类灰色背景，图标直接无背景展示，尽量不要给多余底色，保持干净。
...
- 当前 AI API Key 由 Node 后端使用服务器主密钥加密保存在 SQLite，前端不能读取或持久化明文 Key；涉及安全说明时要同时写清匿名 Cookie、服务端配置和浏览器本地媒体三类边界。
- Docker 已包含前端、Node API、SQLite 与 Nginx 的单机部署文件，但尚未完成正式服务器验收，文档中不要过度承诺生产部署已经验证。
